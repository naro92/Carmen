...
class db {
	...
	function getuserid($user){  //根据病人的名字查找ID   Trouver l'ID par le nom du patient
		$sql = "select `idPatient` from `patient` where `Nom` = '$user'";
	  ...
	  function getfamailleID($userID){   //根据病人的ID查找家庭id  Trouver un famillid par identifiant de patient
		$sql = "SELECT `idFamille` from `famille` where `Patient_idPatient` = '$userID'";
		$tmp = self::$cennct->query($sql); 
		$data = $tmp->Fetch(PDO::FETCH_ASSOC);
...
function getmedecinnom($mi){		//通过病人ID查询对应医生ID  Recherche par ID de patient pour l'ID de médecin correspondant
