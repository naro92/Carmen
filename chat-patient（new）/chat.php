...
$db = db::getdb();
$getpatientnom = $db->getdb()->getpatientnom();
$pi = $db->getdb()->getpatientID($getpatientnom);
$mi = $db->getdb()->getmedecinID($pi);
...
<html>
	<head>
		...
		<script>
			...
			Ajax().get('chat-getlist.php',putinfo);
			}	
	...
	<body>
		...
		<h1 style="position:absolute;top: 13%;left:23%"><?php echo "$getmedecinnom";?></h1>
		<!-- 聊天框 Chat Box-->
		<div id='chat-box' style="overflow:auto;width: 900px;height: 450px;margin: 0 auto; background-color: #FFF;border: 1px solid #DDD;padding: 0px;font-size: 25px;    position: absolute;top: 20%;left: 23%;"></div> 
 		...
 		<!-- 输入栏 Champ de saisie-->
		<input id="chat-input" style="width:600px;height: 60px; border: 1px solid #DDD;padding: 10px;position: absolute;bottom: 15%;left: 28%;font-size: 20px;" type='text' name='say' onkeypress="keyup(event)" />


		<input id="chat-button" style="width: 100px;height: 40px;
    background-color: #C9E265;color: #FFF;border: none; padding: 10px;border-radius: 3px;position: absolute;bottom: 16.3%;
    right: 27%;font-size: 20px;"
